...
- [Flash 'info' dans base_admin — CORRIGÉ](feedback_flash_info_invisible_admin.md) — Désormais rendu (3 boucles flash success/error/info présentes). Vérifier que les futurs templates héritant de base_admin ne court-circuitent pas le block body parent.
...
- [Templates Twig Structure — patterns et anti-patterns identifiés](feedback_twig_structure_patterns.md) — Variables manquantes register (form/alreadyApplied/applicationStatus), double boucle flash info, POST custom sans FormType, pending_validation absent du filtre stats
- [Templates Twig Forum/Messagerie/Notifications/Dashboard — patterns identifiés](feedback_twig_community_patterns.md) — DateTime.timestamp inexistant, pipeline escape|nl2br|raw erroné, is defined vs is not null, double-escaping dans set, format ICU vs PHP date, script inline dans content
- [Templates Twig Admin/Article/OrgProfile — patterns et anti-patterns identifiés](feedback_twig_admin_article_org_patterns.md) — filtre enum partiel contrôleur, form sans action=, CSRF absent article, is defined sur entité, updatedAt nullable, app.user non guardé
